...
Este módulo es **el esquema del contenido editable** de un sitio de cliente de
Velty, dentro del producto `misitio` (`misitio.velty.cl`).

- **Su única razón de ser es que exista UN esquema, no dos.** El panel valida
  con él antes de guardar; el compilador del sitio valida con él antes de
  construir. El contrato entre ambos es **la versión que sus `go.mod`
  declaran**. Si alguna vez alguien copia estos structs a otro repo "para no
  depender", este módulo perdió su motivo de existir.
- **Es agnóstico de plantilla, y eso no es negociable.** No importa
  `tinywasm/layout` ni ninguna otra cosa que renderice. Quien mapea este
  contenido a una plantilla concreta es `veltylabs/sitetheme`, que vive fuera de
  `veltylabs/modules/*` justamente porque sí nombra un renderizador.
- **La composición es fija.** El cliente edita contenido; no elige qué secciones
  aparecen ni en qué orden. Este módulo describe **datos**, nunca disposición.
- **Las unicidades no son un lujo.** `Service.Slug`, `Service.Title` y
  `Service.Description` deben ser únicos dentro del sitio porque
  `layout/landing` entra en pánico ante títulos o descripciones duplicados, y
  ese pánico rompe el build de publicación. Validarlo aquí convierte un build
  roto en un mensaje de formulario.
- **`Images` guarda claves de R2, jamás URLs absolutas.** La URL final la
  construye el tema en el dominio del sitio. Guardar una URL aquí ataría el
  contenido al hosting.
- Este módulo compila **dentro de un Worker de Cloudflare con TinyGo**: sin
  `map[K]V`, sin `encoding/json`, sin `reflect`, sin stdlib pesada. Las
  unicidades se verifican con slices y comparación por pares — son unidades de
  servicios, no miles.
